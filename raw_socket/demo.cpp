...
//        printf("%s", pHostent->h_addr_list[i]);
//    }

//    memcpy(&sa.sin_addr, pHostent->h_addr_list[0], pHostent->h_length);//设置本机地址
    sa.sin_addr.S_un.S_addr = inet_addr("192.168.88.83");

    bind(sock, (SOCKADDR *) &sa, sizeof(sa));//绑定
    if (WSAGetLastError() != 0) {
        printf("happen some error %d", WSAGetLastError());
        exit(0);
    }
    ...
    while (1) {
        ...
        int n = recv(sock, recvBuf, sizeof(recvBuf), 0);//接收包
        if (n < 0) {
            printf("recv error\n");
            return 1;
        }
